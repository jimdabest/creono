            ...
            btnPreview.addEventListener('click', async function() {
                ...
                if (!hasDoc) {
                    ...
                    loading.innerHTML = '<div style="text-align:center;padding:30px 20px;">' +
                        ...
                        '<div style="font-size:13px;color:#94a3b8;">Tính năng Xem trước Watermark chỉ hoạt động với file PDF.<br>' +
                        'Hãy chọn file <strong>.pdf</strong> tại ô "File tài liệu" trước khi bấm xem trước.</div>' +
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        // Nếu server tạo được file PDF hoàn chỉnh qua FPDI
                        if (result.mode === 'server_pdf' && result.preview_url) {
                            ...
                            // Chế độ Canvas Watermark (fallback qua PDF.js)
                            try {
                            ...
                            // Fallback ảnh trực tiếp từ GD Library
                            renderFallbackImages(result.preview_images);
                        } else {
