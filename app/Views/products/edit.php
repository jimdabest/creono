...
<div class="container page-container" style="margin-top: 40px; margin-bottom: 80px; max-width: 720px;">
    <h1 style="font-size: 32px; font-weight: 700; letter-spacing: -0.02em; margin-bottom: 8px; color: #1d1d1f;">Chỉnh sửa sản phẩm</h1>
    <p style="font-size: 16px; color: #86868b; margin-bottom: 32px;">Cập nhật thông tin tài liệu của bạn.</p>
        ...
        <form action="<?php echo URLROOT; ?>/products/edit/<?php echo $data['product']->id; ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="category_id" id="category_id" class="form-control <?php echo isset($data['errors']['category_err']) ? 'is-invalid' : ''; ?>" required>
                    ...
                    <?php foreach ($data['categories'] as $cat) : ?>
                        <option value="<?php echo $cat->id; ?>" <?php echo ($data['product']->category_id == $cat->id) ? 'selected' : ''; ?>><?php echo htmlspecialchars($cat->name); ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <?php if (!empty($data['product']->preview_url)) : ?>
                    <div style="margin-bottom: 8px;"><img src="<?php echo URLROOT . htmlspecialchars($data['product']->preview_url); ?>" style="max-width: 120px; border-radius: 8px; border: 1px solid #ddd; padding: 4px;"></div>
                <?php endif; ?>
            ...
            <div class="form-group">
                ...
                <?php if (!empty($data['document']->file_url)) : ?>
                    <div style="margin-bottom: 8px; font-size: 14px; color: #0071e3;">File hiện tại: <a href="<?php echo URLROOT . htmlspecialchars($data['document']->file_url); ?>" target="_blank">Tải xuống</a></div>
                <?php endif; ?>
            ...
            <div style="margin: 18px 0; padding: 14px 18px; background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 14px; display: flex; align-items: center; justify-content: space-between; gap: 12px;">
                ...
                <button type="button" id="btnSellerPreview" style="padding: 9px 18px; border-radius: 10px; background: #ffffff; border: 1.5px solid #0071e3; color: #0071e3; font-weight: 600; font-size: 13px; cursor: pointer; display: flex; align-items: center; gap: 6px; white-space: nowrap;">Xem trước Watermark</button>
            </div>
